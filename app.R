...
ui <- fluidPage(
  ...
  navbarPage("Dashboard",
             ...
             tabPanel("Gastos",
                      ...
                      fluidRow(
                        column(5, 
                               ...
                               verbatimTextOutput("contents_lugar_comp")
                        ),
                        column(7, 
                      ...
                      fluidRow(
                        column(5, 
                               fileInput("file_tipo_gasto", "Archivo para Mes:",
                                         accept = c(".txt", ".csv", ".xlsx", ".docx")),
                               verbatimTextOutput("contents_tipo_gasto")
                        ),
                        column(7, 
                      ...
                      fluidRow(
                        column(5, 
                               fileInput("file_folio_hog", "Archivo para Producto:",
                                         accept = c(".txt", ".csv", ".xlsx", ".docx")),
                               verbatimTextOutput("content_folio_hog")
                        ),
                        column(7, 
             ...
             tabPanel("INPC por pais",
                      h3("Índice nacional de precios al consumidor por país"),
                      plotOutput("plotweb")
             ),
...
server <- function(input, output,session) {
  ...
  output$contents_lugar_comp <- renderPrint({
    ...
    tryCatch({
      mongo_conexionLC$insert(content_lugar_comp)
    }, error = function(e) {
  ...
  output$contents_tipo_gasto <- renderPrint({
    ...
    if (ext == "txt") {
      ...
      content_tipo_gasto <- read.csv(input$file_tipo_gasto$datapath, stringsAsFactors = FALSE, colClasses = c("tipo_gasto" = "character"))
    ...
    tryCatch({
      mongo_connectionMZ$insert(content_tipo_gasto)
    }, error = function(e) {
      cat("Error al insertar datos en MongoDB:", e$message, "\n")
    })
  })
  ...
  output$content_folio_hog <- renderPrint({
    ...
    if (ext == "txt") {
      ...
      content_folio_hog <- read.csv(input$file_folio_hog$datapath, stringsAsFactors = FALSE, colClasses = c("foliohog" = "character"))
    ...
    tryCatch({
      mongo_connectionPR$insert(content_folio_hog)
    }, error = function(e) {
      cat("Error al insertar datos en MongoDB:", e$message, "\n")
    })
  })
  ...
  # Usar reactiveTimer para ejecutar el scraping cada 5 segundos
